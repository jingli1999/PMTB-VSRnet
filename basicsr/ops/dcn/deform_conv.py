...
class DeformConvPack(DeformConv):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.init_offset()
        # 修改后：新增：偏移限幅参数与缓存
        self.max_offset = 12.0  # 可在上层根据金字塔层级覆盖，如 L3:4, L2:6, L1:12
        self.last_offset = None  # 训练时供 L_off 正则读取（不反传）
    ...
    #        offset = torch.tanh(offset / self.max_offset) * self.max_offset
    ...
    def forward(self, x):
        # 1) 正常算 offset
        offset = self.conv_offset(x)  # 已有

        # 2) 新增：tanh 限幅（支持从外部设置 self.max_offset，默认 12 像素）
        max_off = float(getattr(self, 'max_offset', 12.0))
        max_off = max(max_off, 1e-6)  # 防止除零
        offset = torch.tanh(offset / max_off) * max_off

        # 3) 新增：缓存这一次的 offset 供 L_off 正则用
        self.last_offset = offset.detach()

        # 4) 原有的 deformable conv
...
class ModulatedDeformConvPack(ModulatedDeformConv):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.init_weights()
        # ---修改后： 新增：偏移限幅参数与缓存 ---
        self.max_offset = 12.0  # AMSR2 推荐 6~12（L1 尺度）；可按层覆写
        self.last_offset = None  # 训练时供 L_off 正则读取（不反传）
    ...
    #     max_off = getattr(self, 'max_offset', 12.0)
    #     offset = torch.tanh(offset / max_off) * max_off
    #     # 可选：按组做平滑或做与 DeformConvPack 同步的 last_offset 缓存
